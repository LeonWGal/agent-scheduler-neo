...
from agent_scheduler.forge_neveroom_compat import apply_neveroom_compat

# Durable NeverOOM / inference-tensor fix lives in the extension (survives Forge updates).
apply_neveroom_compat()
...
def enqueue_button_js(is_img2img: bool) -> str:
    """Self-contained Gradio frontend JS — must NOT depend on extension iife load order."""
    prefix = "img2img" if is_img2img else "txt2img"
    mode_fix = (
        "out[2] = (typeof get_tab_index === 'function') ? get_tab_index('mode_img2img') : out[2];"
        if is_img2img
        else ""
    )
    # Use gradioApp() when present (Forge shadow root). Never throw — backend must still run.
    return f"""
(...args) => {{
  try {{
    const root = (typeof gradioApp === 'function') ? gradioApp() : document;
    const out = (args.length === 1 && Array.isArray(args[0])) ? Array.from(args[0]) : Array.from(args);
    const wrap = root.querySelector('#{prefix}_enqueue_wrapper input');
    const setting = root.querySelector('#setting_sd_model_checkpoint input');
    let ckpt = (wrap && wrap.value) || (setting && setting.value) || 'Current Checkpoint';
    if (!ckpt) ckpt = 'Current Checkpoint';
    out[0] = ckpt;
    const rid = (typeof randomId === 'function')
      ? randomId()
      : ('task(' + Math.random().toString(36).slice(2, 7) + Math.random().toString(36).slice(2, 7) + Math.random().toString(36).slice(2, 7) + ')');
    out[1] = rid;
    {mode_fix}
    const btn = root.querySelector('#{prefix}_enqueue');
    if (btn) {{
      btn.innerText = 'Queued';
      setTimeout(() => {{ btn.innerText = 'Enqueue'; }}, 1000);
    }}
    return out;
  }} catch (e) {{
    console.error('[AgentScheduler] enqueue js failed', e);
...
class Script(scripts.Script):
    ...
    def add_enqueue_button(self):
        ...
        with gr.Row(elem_id=f"{id_part}_enqueue_wrapper") as row:
            ...
            # Gradio 4 needs a real output for click→backend roundtrip (empty outputs=[] can skip).
            self.enqueue_status = gr.HTML(value="", visible=False, elem_id=f"{id_part}_enqueue_status")
    ...
    def bind_enqueue_button(self, root: gr.Blocks):
        ...
        for d in dependencies:
            if len(d["outputs"]) == 1:
                ...
                if outputs:
                    output = outputs[0]
                    if isinstance(output, gr.State) and type(output.value).__name__ in (
                        "UiControlNetUnit",
                        "ControlNetUnit",
                    ):
                        cnet_dependency = d
        ...
        with root:
            ...
            status_out = getattr(self, "enqueue_status", None)
            self.submit_button.click(
                fn=fn,
                js=enqueue_button_js(is_img2img),
                inputs=new_inputs,
                outputs=[status_out] if status_out is not None else None,
    ...
    def wrap_register_ui_task(self):
        def f(request: gr.Request, *args):
            ...
            # Prefer JS-prepared payload: [checkpoint, task_id, ...generate]
            if len(args) >= 2 and isinstance(args[1], str) and (
                args[1].startswith("task(")
                or args[1] == queue_with_every_checkpoints
                or (len(args[1]) > 8 and "-" in args[1])
            ):
                checkpoint = args[0] if isinstance(args[0], str) else checkpoint
                task_id = args[1]
                gen_args = args[1:]  # keep task_id as first generate arg
            elif isinstance(args[0], str) and (
                args[0].startswith("task(") or args[0] == queue_with_every_checkpoints
            ):
                # Checkpoint dropdown skipped/hidden in payload
                task_id = args[0]
                gen_args = args
            else:
                # Fallback: treat args[0] as task id slot (may be empty dummy)
                task_id = args[0] if isinstance(args[0], str) and args[0] else str(uuid4())
                gen_args = args
                log.warning(
                    f"[AgentScheduler] Enqueue JS payload missing task id; head={args[:3]!r}"
                )
            ...
            if task_id == queue_with_every_checkpoints:
                ...
                if not isinstance(task_id, str) or not task_id.startswith("task("):
                    task_name = str(task_id) if task_id else None
                    task_id = str(uuid4())
                    # Ensure generate args still start with an id slot
                    if gen_args:
                        gen_args = [task_id] + list(gen_args[1:])
                    else:
                        gen_args = [task_id]
                ...
                if checkpoint is None or checkpoint == "" or checkpoint == checkpoint_current:
                    ...
                    checkpoint = [None]
                elif isinstance(checkpoint, str) and checkpoint.endswith(" checkpoints)"):
            ...
            log.info(
                f"[AgentScheduler] register enqueue task_id={task_id} "
                f"checkpoint={checkpoint} n_args={len(gen_args)}"
            )
            ...
            for i, c in enumerate(checkpoint):
                ...
                try:
                    ...
                    log.error(f"[AgentScheduler] Failed to register UI task {t_id}: {e}")
                    import traceback

                    log.error(traceback.format_exc())

            task_runner.execute_pending_tasks_threading()
            return f"<span data-enqueued='{task_id}'></span>"
...
def get_task_results(task_id: str, image_idx: int = None):
    ...
    if task is None:
        ...
        try:
            ...
            # Resolve relative paths saved by older builds
            resolved = []
            for p in images:
                if not p:
                    continue
            ...
            # Gradio Update mistakenly stored as geninfo (Forge Neo return-index bug)
            if isinstance(geninfo, dict) and geninfo.get("__type__") == "update":
                geninfo = None
            ...
            if isinstance(geninfo, dict):
                ...
                geninfo = infotexts_to_geninfo(infotexts) if infotexts else None

            if not infotexts and images:
                from agent_scheduler.task_helpers import geninfo_from_saved_images

                rebuilt = geninfo_from_saved_images(images)
                if rebuilt:
                    geninfo = rebuilt
                    infotexts = rebuilt.get("infotexts", [])

            galerry = [Image.open(i) for i in images] if image_idx is None else gr.update()
            idx = image_idx if image_idx is not None else 0
            if idx < len(infotexts):
                infotext = infotexts[idx]
            elif images and not infotext:
                infotext = f"Images: {len(images)}"
        except Exception as e:
...
def on_ui_tab(**_kwargs):
    ...
    with gr.Blocks(analytics_enabled=False) as scheduler_tab:
        ...
        status.change(
            fn=lambda x: None,
            js="agent_scheduler_status_filter_changed",
            inputs=[status],
        )
        save.click(
            fn=lambda x, y, z: call_queue.wrap_gradio_call(save_files)(x, y, False, int(z)),
            js="(x, y, z) => [x, y, selected_gallery_index()]",
...
def on_app_started(block: gr.Blocks, app):
    global task_runner
    # Re-apply after full backend init (idempotent).
    apply_neveroom_compat()
