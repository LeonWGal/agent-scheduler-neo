...
def regsiter_apis(app: App, task_runner: TaskRunner):
    ...
    class UiQueueBody(BaseModel):
        """Raw Gradio UI generate args (same list as txt2img/img2img submit)."""

        is_img2img: bool = False
        checkpoint: Optional[str] = None
        args: List[object]

    @app.post("/agent-scheduler/v1/queue/ui", response_model=QueueTaskResponse, dependencies=deps)
    def queue_ui(body: UiQueueBody):
        """Enqueue from UI bridge — bypasses Gradio preprocess (late-bound enqueue deps)."""
        args: List = list(body.args or [])
        if not args:
            raise HTTPException(status_code=400, detail="Empty args")

        task_id = args[0] if isinstance(args[0], str) and str(args[0]).startswith("task(") else str(uuid4())
        if not (isinstance(args[0], str) and str(args[0]).startswith("task(")):
            args = [task_id] + args

        checkpoint = body.checkpoint
        if checkpoint in (None, "", "Current Checkpoint"):
            checkpoint = None

        log.info(
            f"[AgentScheduler] UI queue bridge: task_id={task_id} "
            f"img2img={body.is_img2img} n_args={len(args)} checkpoint={checkpoint}"
        )
        ...
        try:
            ...
            log.error(f"[AgentScheduler] UI queue bridge failed: {e}")
    ...
    def get_task_results(id: str, zip: Optional[bool] = False):
        ...
        for image in result.get("images", []):
            ...
            path = Path(image)
            if not path.is_file():
                path = Path(os.path.abspath(image))
            if path.is_file():
        ...
        if infotexts is None:
            ...
            if isinstance(geninfo, dict) and geninfo.get("__type__") == "update":
                geninfo = {}
            infotexts = geninfo.get("infotexts") if isinstance(geninfo, dict) else None
            if not infotexts:
                from .task_helpers import geninfo_from_saved_images

                rebuilt = geninfo_from_saved_images([str(p) for p in image_paths])
                infotexts = rebuilt.get("infotexts", []) if rebuilt else []
            if not infotexts:
                infotexts = defaultdict(lambda: "")
        ...
        if zip:
            ...
            for i, path in enumerate(image_paths):
                if isinstance(infotexts, defaultdict):
                    text = infotexts[i]
                elif isinstance(infotexts, list) and i < len(infotexts):
                    text = infotexts[i]
                else:
                    text = ""
                data.append(
                    {
                        "image": encode_image_to_base64(Image.open(path)),
                        "infotext": text,
                    }
                )
