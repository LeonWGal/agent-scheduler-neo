...
def detect_control_net(root: gr.Blocks, submit: gr.Button):
    ...
    cnet_type_names = ("UiControlNetUnit", "ControlNetUnit")
    ...
    for d in dependencies:
        if len(d["outputs"]) == 1:
            ...
            if (
                ...
                and type(output.value).__name__ in cnet_type_names
            ):
...
def request_with_retry(
    ...
    try:
        ...
        log.error("[AgentScheduler] Connection error while uploading result")
        if retries >= max_try - 1:
            return False

        time.sleep(1)
        log.info(f"[AgentScheduler] Retrying {retries + 1}...")
        ...
        log.error("[AgentScheduler] Error while uploading result")
        log.error(e)
