...
from .forge_neveroom_compat import prepare_for_queued_inference
from .helpers import (
    log,
    detect_control_net,
...
from .task_helpers import (
    ...
    extract_ui_task_error,
    geninfo_from_saved_images,
    normalize_saved_image_paths,
    normalize_adetailer_script_args,
    normalize_xyz_grid_script_args,
)
...
class TaskRunner:
    ...
    def __serialize_ui_task_args(
        ...
        *args,
        ...
        # Coerce null ADetailer State slots before pickle (enqueue bridge should already fill them)
        script_args = normalize_adetailer_script_args(script_args, is_img2img=is_img2img)
        # XYZ plot x/y/z type Dropdown(type=index) may arrive as label strings
        ...
        if "request" in named_args:
            ...
            named_args["request"] = {"username": username}
    ...
    def __deserialize_ui_task_args(
        ...
        script_args = deserialize_script_args(script_args, self.UiControlNetUnit, is_img2img=is_img2img)
    ...
    def __deserialize_api_task_args(
        ...
        script_args = deserialize_script_args(script_args, is_img2img=is_img2img)
        return (named_args, script_args)
    ...
    def execute_task(self, task: Task, get_next_task: Callable[[], Task]):
        while True:
            ...
            if progress.current_task is None:
                ...
                if not res or isinstance(res, Exception):
                    ...
                    if getattr(shared.opts, "queue_automatic_requeue_failed_task", False):
                        ...
                        if isinstance(res, Exception):
                            task.result = f"{type(res).__name__}: {res}"
                            if not str(res) or str(res) == "None":
                                task.result = f"{type(res).__name__}: {repr(res)}"
                        else:
                            task.result = str(res) if res else "Unknown error (no exception captured)"
                        task_manager.update_task(task)
                    ...
                    if is_interrupted:
                        ...
                        images = normalize_saved_image_paths(self.__saved_images_path)
                        geninfo = extract_generation_info(res)
                        if not isinstance(geninfo, dict) or not geninfo.get("infotexts"):
                            fallback = geninfo_from_saved_images(images)
                            if fallback:
                                geninfo = fallback
                        result = {
                            "images": images,
                            "geninfo": geninfo if isinstance(geninfo, dict) else {},
                        }
                ...
                # Wait for concurrent WebUI generation to finish (poll, don't sleep 2s blind)
                for _ in range(20):
                    if progress.current_task is None:
                        break
                    time.sleep(0.1)
            ...
            if not task:
                if not self.paused:
                    time.sleep(0.5)
                    # Re-check once — a task may have been enqueued during the wait
                    task = get_next_task()
                    if task:
                        continue
                    self.__on_completed()
    ...
    def execute_pending_tasks_threading(self):
        ...
        if self.is_executing_task:
            log.debug("[AgentScheduler] Runner already running")
            return
    ...
    def __execute_ui_task(self, task_id: str, is_img2img: bool, *args):
        # Ensure NeverOOM inference-tensor demotion hooks are active before TE/CLIP embed
        prepare_for_queued_inference()

        # Match Forge Neo Generate outputs so wrap_gradio error HTML lands in the right slot
        extra_outputs = [None, None, "", ""]
        func = wrap_gradio_call(img2img if is_img2img else txt2img, extra_outputs=extra_outputs, add_stats=True)
        ...
        with queue_lock:
            ...
            res = None
            try:
                main_thread.last_exception = None
                result = func(*args)

                # Forge main_thread swallows exceptions → result None / HTML error; recover real message
                ui_error = extract_ui_task_error(result, main_thread.last_exception)
                oom_msg = ""
                if isinstance(result, (list, tuple)):
                    for part in result:
                        if isinstance(part, str) and "CUDA out of memory" in part:
                            oom_msg = part
                            break

                if ui_error:
                    if "OOM" in ui_error or "out of memory" in ui_error.lower():
                        res = OutOfMemoryError(ui_error)
                    else:
                        res = RuntimeError(ui_error)
                elif (result and result[0] is None and hasattr(shared.state, "oom") and shared.state.oom) or oom_msg:
                    res = OutOfMemoryError(oom_msg or "CUDA out of memory")
                else:
                    res = extract_generation_info(result)
                    if res is None and result is not None:
                        # Generation returned but no geninfo — treat empty gallery + error HTML as failure
                        err = extract_ui_task_error(result, None)
                        if err:
                            res = RuntimeError(err)
            except Exception as e:
                if main_thread.last_exception:
                    msg = main_thread.last_exception
                    res = OutOfMemoryError(msg) if msg == "OOM" or "out of memory" in msg.lower() else RuntimeError(msg)
                else:
                    res = e
            finally:
    ...
    def __on_image_saved(self, data: script_callbacks.ImageSaveParams):
        ...
        filename = os.path.abspath(data.filename) if data.filename else data.filename
        outpath_grids = shared.opts.outdir_grids or shared.opts.outdir_txt2img_grids
        if outpath_grids and filename.startswith(os.path.abspath(outpath_grids)):
            self.__saved_images_path.insert(0, filename)
        else:
            self.__saved_images_path.append(filename)
