...
Historical notes from the upstream project (issue numbers refer to the original tracker).

## 2023/08/10

New features:
- New API `/task/{id}/position` to get task position in queue (#105)
- Display task in local timezone (#95)

Bugs fixing:
- `alwayson_scripts` should allow script name in all cases (#102)
- Fix `script_args` not working when queue task via API (#103)

## 2023/08/02

Bugs fixing:
- Fix task_id is duplicated (#97)
- Fix (#100)
...
- Queue task with specific name & queue with all checkpoints (#88)

## 2023/07/24

New features:
- New API `/task/{id}` to get single task (#86)
- Update queued task
...
- Fix task re-ordering not working (#79)
